...
export default function DetailsPage(props) {
  ...
  useEffect(()=>{
    ...
    const handleRemoveCourse = (index)=>{
      if (index > -1) {
      instance
        .delete(`api2/cart/${arr[index].id}`)
        .then((res) => {
          try {
            const newArr = [...arr];
            const reArr = newArr.splice(index, 1);
            setArr(newArr);
            dispatch({ type: "REMOVE", item: reArr[0] });
            toast.success("Item removed from your cart");
          } catch {
            toast.error("Unknown Error Occored");
          }
        })
...
const handlePayement = ()=>{
    instance
        .post("api2/strip-payment/", { orderRef: orderRef })
        .then((res) => {
            ...
            <GridContainer>
              <Grid container spacing={4} className={classesTwo.productgrid}>
                <Grid item xs={12} sm={8}>
                      ...
                      >
                        ...
                        <Grid item xs={12} sm={2}>
                          <Typography
                            ...
                            className={classesTwo.subtitle1}
                          >
                            {obj.item__price > obj.item__discount_price
                              ? obj.item__discount_price
                              : obj.item__price}
                          </Typography>
                          <Button
                            style={{ marginTop: 15 }}
                            size="sm"
                            color="danger"
                            className={classesTwo.cartbutton}
                  ...
                  {/* -------------------- check out part of the cart page --------------------*/}
                </Grid>
                <Grid item xs={12} sm={4}>
                  ...
                  <Box border={2} className={classesTwo.roundedbox}>
                  ...
                  <div className={classesTwo.bigspacer} />
                  <TextField
                    id="filled-dense-multiline"
                    label="promo code"
                    className={clsx(classesTwo.textField, classesTwo.dense)}
                    margin="dense"
                    variant="filled"
                    multiline
                    rowsMax="4"
